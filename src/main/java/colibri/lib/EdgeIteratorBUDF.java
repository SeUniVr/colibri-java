...
class EdgeIteratorBUDF implements Iterator<Edge> {
	...
	private final LinkedList<Set<ComparableSet>> past;	//stores the attribute sets of the concepts that have already been completely explored
	private final LinkedList<Concept> current;			//stores the current path
	private final LinkedList<Iterator<Concept>> future;	//stores the concepts that will be considered in the future
